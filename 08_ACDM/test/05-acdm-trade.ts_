...
describe("ACDM Platform - Trade round", function () {
  ...
  describe("Redeem order", function () {
    it("Should update token balanses", async function () {
      ...
      await expect(() =>
        acdmPlatform.connect(user2).redeemOrder(0, { value: 1000 * 10000000 })
    ...
    it("Should update token balanses after partial redemption", async function () {
      ...
      await expect(() =>
        acdmPlatform.connect(user2).redeemOrder(0, { value: 500 * 10000000 })
      ).to.changeTokenBalances(acdmToken, [acdmPlatform, user2], [-500, 500]);

      await expect(() =>
        acdmPlatform.connect(user3).redeemOrder(0, { value: 500 * 10000000 })
    ...
    it("Should update eth balanses", async function () {
      ...
      await expect(
        await acdmPlatform.connect(user2).redeemOrder(0, { value: 10 * 1000 })
      ).to.changeEtherBalance(user2, -(10 * 1000));
    });
    ...
    it("Should deposit referral rewards after redeem", async function () {
      ...
      let amount = 10 * 1000;
      let ref1Amount = (amount * ref1Percent) / 1000;
      let ref2Amount = (amount * ref2Percent) / 1000;

      await expect(
        await acdmPlatform.connect(deployer).redeemOrder(0, { value: amount })
      ).to.changeEtherBalances(
        [user3, user2, user1],
        [amount - ref1Amount - ref2Amount, ref1Amount, ref2Amount]
      );
    });
    ...
    it("Should change tradeRoundVolume after redeem", async function () {
      ...
      let amount = 10 * 1000;
      await acdmPlatform.connect(deployer).redeemOrder(0, { value: amount });
      expect(await acdmPlatform.tradeRoundVolume()).to.eq(amount);
    });
  });
  ...
  describe("Start sale round", function () {
    ...
    it("Should refund tokens from unexecuted orders", async function () {
      await acdmPlatform.connect(user1).addOrder(10, 1000);
      await acdmPlatform.connect(user2).addOrder(20, 1000);

      await acdmPlatform.connect(user3).redeemOrder(1, { value: 10 * 1000 });

      let roundDuration = 3 * 24 * 60 * 60;
      await network.provider.send("evm_increaseTime", [roundDuration + 1]);

      await expect(() => acdmPlatform.startSaleRound()).to.changeTokenBalances(
        acdmToken,
        [user1, user2],
        [10, 10]
      );
